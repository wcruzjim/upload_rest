...
abstract class REST_Controller extends CI_Controller {
    ...
    private function get_request_jwt_data(){

        $request_data = array(
            "request_jwt_decoded" => (object)array(),
            "request_jwt_raw" => NULL,
        );

        $raw_token = \SECURITY::get_raw_token();

        if(  !isset( $raw_token ) || empty( $raw_token )  ){
            return $request_data;
        }

        try{
            $request_data['request_jwt_decoded'] = \SECURITY::getJwtData();
            $request_data['request_jwt_raw'] = \SECURITY::get_raw_token();
        }
        catch(Exception $e){
            // commented line
        }
    ...
    protected function _log_request($authorized = FALSE)
    {
        ...
        if($this->uri->uri_string() == 'seguridad/login'){
            return false;
        }

        $request_token_data = $this->get_request_jwt_data();
        $raw_token = $request_token_data['request_jwt_raw'];
        $decoded_token = $request_token_data['request_jwt_decoded'];

        $is_inserted = $this->rest->db
            ->insert(
                $this->config->item('rest_logs_table'), [
                'route' => implode('/', array_slice(explode('/', $this->uri->uri_string()), 0, 2)),
                ...
                'time' => time(),
                'date' => date('Y-m-d'),
                'document' => property_exists($decoded_token, 'id') ?  $decoded_token->id : NULL ,
                'simulated' => property_exists($decoded_token, 'simulated') ?  $decoded_token->simulated : 0 
            ]);
    ...
    protected function _check_login($username = NULL, $password = FALSE)
    {
        ...
        if ( ! $this->config->item('auth_source') && $rest_auth === 'digest')
        {
            ...
            return hash("sha256",$username.':'.$this->config->item('rest_realm').':'.(isset($valid_logins[$username]) ? $valid_logins[$username] : ''));
        }
    ...
    protected function _prepare_digest_auth()
    {
        ...
        $md5 = hash("sha256",strtoupper($this->request->method).':'.$digest['uri']);

        $valid_response = hash("sha256",$digest['username'].':'.$digest['nonce'].':'.$digest['nc'].':'.$digest['cnonce'].':'.$digest['qop'].':'.$md5);
    ...
    protected function _log_access_time()
    {
        ...
        $payload['conexiones_db'] = \COMMON::get_counter_queries_database();
        $payload['sesiones_db'] = \COMMON::get_counter_sessions_database();
    ...
    protected function _check_cors()
    {
        ...
        { // solo entra si la variable de configuración allow_any_cors_domain es seteada en true, en este momento está desactivada
     ...
     * @return bool TRUE the API key has access; otherwise, FALSE
     */
    protected function decodePayloadJWT($jwt)
    {
        $tk = explode('.', $jwt);
        if(isset($tk[1])){
            if(base64_decode($tk[1]) !== FALSE){
                return json_decode(base64_decode($tk[1]));
            }
            return FALSE;
        }
        return FALSE;
